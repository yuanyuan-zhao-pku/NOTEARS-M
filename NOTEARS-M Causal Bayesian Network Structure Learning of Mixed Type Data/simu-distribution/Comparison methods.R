...
for (n in c(1000)) {  
  for (distribution in c("gauss", "exp", "gumbel")) {   
    ...
    filenameB <- file.path(default_save_path, sprintf("B_true_n%d_distribution%s.csv", n, distribution))
    B_true <- read.csv(filenameB, header = FALSE) %>% as.matrix()  
    
    for (i in 0:9) {  
      print(paste0("i=", i))  
      
      filenameWE <- file.path(default_save_path, sprintf("W_est_n%d_distribution%s_i%d.csv", n, distribution, i))  
      filenameX <- file.path(default_save_path, sprintf("X_n%d_distribution%s_i%d.csv", n, distribution, i))        
...
write.csv(results_avg, file.path(default_save_path, "acc_distribution_MMHC.csv"), row.names = FALSE)   
write.csv(results, file.path(default_save_path, "each_acc_distribution_MMHC.csv"), row.names = FALSE)   
